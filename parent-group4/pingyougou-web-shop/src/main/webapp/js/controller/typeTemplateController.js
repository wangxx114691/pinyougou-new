...
app.controller('typeTemplateController' ,function($scope,$controller,brandService ,specificationService  ,typeTemplateService){	
	...
	$scope.findAll=function(page,rows){
        typeTemplateService.findAll(page,rows,$scope.searchEntity).success(
            function(response){
    ...
    $scope.findOne2=function(id){
        typeTemplateService.findOne2(id).success(
            function(response){
                $scope.entity= response;
                // eval()   JSON.parse();
                $scope.entity.brandIds = JSON.parse($scope.entity.brandIds);
    ...
    $scope.save2=function(){
        var serviceObject;//服务层对象
        if($scope.entity.id!=null){//如果有ID
            serviceObject=typeTemplateService.update2( $scope.entity ); //修改
        }else{
            serviceObject=typeTemplateService.add2( $scope.entity  );//增加
        }
        serviceObject.success(
            function(response){
                if(response.flag){
	...
	$scope.findBrandList = function(){
		brandService.selectOptionList().success(function(response){
			...
			// 将查出的结果转换为brandList中的data值, 但是必须是{id:.. ,text:..} 但是这样就和原数据库中表的结果不一致, 所以需要查询的时候再封装给List<Map>
		});
	}
